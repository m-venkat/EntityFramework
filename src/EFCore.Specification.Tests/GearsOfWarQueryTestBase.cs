...
#pragma warning restore CS0458 // The result of the expression is always 'null'
        ...
        [ConditionalFact]
        public virtual void OfType_result_operator_overrides_Cast_that_comes_before()
        {
            using (var ctx = CreateContext())
            {
                var query = from lh in (from f in ctx.Factions
                                        select f).Cast<Faction>().OfType<LocustHorde>()
                            select lh.Eradicated;
        ...
        [ConditionalFact]
        public virtual void When_lifting_multiple_Cast_operators_last_one_wins()
        {
            using (var ctx = CreateContext())
            {
                var query = from lh in (from f in ctx.Factions
                                        select f).Cast<Faction>().Cast<LocustHorde>()
                            select lh.Eradicated;
